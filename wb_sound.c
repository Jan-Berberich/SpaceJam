...
void wbSoundUninit(WBSound* sound) {
    if (sound->gameover) ma_sound_uninit(&game->sound.gameover);
    if (sound->wizdeath) ma_sound_uninit(&game->sound.wizdeath);
    if (sound->cathit) ma_sound_uninit(&game->sound.cathit);
    if (sound->bomb) ma_sound_uninit(&game->sound.bomb);
    if (sound->blinker) ma_sound_uninit(&game->sound.blinker);
    if (sound->clear) ma_sound_uninit(&game->sound.clear);
    if (sound->decay) ma_sound_uninit(&game->sound.decay);
    if (sound->powerup_activate) ma_sound_uninit(&game->sound.powerup_activate);
    if (sound->powerup_collect) ma_sound_uninit(&game->sound.powerup_collect);
    if (sound->powerup_drop) ma_sound_uninit(&game->sound.powerup_drop);
    if (sound->fire_spam) ma_sound_uninit(&game->sound.fire_spam);
    if (sound->fire) ma_sound_uninit(&game->sound.fire);
    if (sound->getready) ma_sound_uninit(&game->sound.getready);
    if (sound->titlescreen) ma_sound_uninit(&game->sound.titlescreen);
    if (sound->_engine) ma_engine_uninit(&game->sound._engine);
}