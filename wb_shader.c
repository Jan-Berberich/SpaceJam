...
void wbShaderInit(WBShader* shader) {
    ...
    static const char* fragment_shader_src =
        ...
        "        switch (keyColorMode) {"
        "            case 0:"
        "            idx = int(mod(replaceColorSpeed * frameCnt, replaceColorCnt));"
        "            FragColor = replaceColors[idx];"
        "            break;"
        "            case 1:"
        "            float row_offset = abs(gl_FragCoord.y * windowScale - replaceColorReflectHeight);"
        "            idx = int(mod(((row_offset + replaceColorReflectHeight) / subpixelCnt -"
        "                floor(replaceColorSpeed * frameCnt)) / replaceColorHeight, replaceColorCnt));"
        "            FragColor = replaceColors[idx];"
        "            break;"
        "            default:"
        ...
        "        float flicker = 0.4 * sin(200.0 * pixel.x + 300.0 * pixel.y + 30.0 * deltaTime * frameCnt)"
        "                      + 0.2 * sin(150.0 * pixel.x - 100.0 * pixel.y - 10.0 * deltaTime * frameCnt)"
        "                      + 0.3 * sin(-50.0 * pixel.x -  30.0 * pixel.y +  5.0 * deltaTime * frameCnt)"
        "                      - 0.5 * sin(-30.0 * pixel.x +  10.0 * pixel.y -  1.0 * deltaTime * frameCnt);"
